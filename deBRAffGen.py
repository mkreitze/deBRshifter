...
def gen_initWindow(W,VERBOSE = False):
  W0 = np.zeros(W)
  W0[-1,-1] = 1
  W0 = np.append(W0,1) # since we are dealing with affine system
...
#OUTPUT allShifters (list of all circ combo shifters as numpy arrays, with their make up circs as a list, last digit represents the affine used)
def gen_allShifter(A,W):
  allShifters = []
  genericShifter = gen_shifter(W)
  Cs = gen_subcircs(A,W)
  for j in range(A): # added for affine
    ...
    for circCombo in circCombos:
      ...
      circComp.append(j) # added for affine
      for i in range(W[1] - len(to_base(circNum,A**W[0]))):
        circComp.insert(0,0)
      shifter = np.copy(genericShifter) # Matt paranoia
      shifter[W[0]:W[0]*W[1],0:W[0]*W[1]] = np.concatenate(circCombo, axis = 1) # can use -W[0],: if not affine
      shifter[0:W[0]*W[1],-1] = j # added in for affine
      shifter[-1,-1] = 1 # added in for affine
